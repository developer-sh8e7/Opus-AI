...
export class ErrorHandler {
  ...
  private errorPatterns: Record<string, {
    ...
    rate_limit: {
      ...
      solution: 'الانتظار قبل المحاولة مجدداً',
      severity: 'medium'
    },
    memory: {
  ...
  handleError(error: Error, context?: Record<string, any>): ErrorLog {
    console.error('[ErrorHandler] خطأ:', error.message);
    ...
    if (solution) {
      console.log(`[ErrorHandler] الحل المقترح: ${solution}`);
    }
...
${emoji} ════════════════════════════════════════════
  ...
  clearLogs(): void {
    this.errorLogs = [];
    console.log('[ErrorHandler] تم إعادة تعيين سجل الأخطاء');
  }
  ...
  /**
   * محاولة إعادة المحاولة مع التأخير
   */
  async retryWithBackoff<T>(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        console.warn(`[ErrorHandler] المحاولة ${attempt}/${maxRetries} فشلت: ${lastError.message}`);
